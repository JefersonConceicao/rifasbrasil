...
function getVencedor($rifa_codigo, $bilhete, $mysqli){
	...
	if($rifa_codigo > 0 && $bilhete > 0){
		...
		AND (bil_situacao = 'V' or bil_situacao = 'P')";
		...
		AND (bil_situacao = 'V' or bil_situacao = 'P')) LIMIT 1";

			$usuario = $mysqli->query($code_usu)->fetch_assoc();


			//$usuario = DBSelect($code_usu, $mysqli);

			echo $usuario['comp_cliente'];
		}
