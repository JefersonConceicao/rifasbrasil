...
function get_cod_alfanumerico($mysqli, $rifa, $bil_cod){
	...
	*/
	...
	$tmp = explode('-', file_get_contents('http://52.67.27.97/new_server/get_bilhete.php?rifa='.$rifa.'&layout=1&bilhete='.$bil_cod));
...
$bilhetes = DBSelect("select * from tbl_bilhetes where bil_numero = '$bilhete' and bil_rifa = '$rifa'", $mysqli);
...
elseif(count($bilhetes) == 0){
	...
	// Fim
	$comprador = DBSelect("select * from tbl_compra c, tbl_usuario u where c.comp_cod = '".$bilhetes['bil_compra']."' and u.usu_cod = c.comp_cliente limit 1", $mysqli);
