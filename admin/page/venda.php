...
if(isset($_SESSION['saved_post_enabled'])) {
    ...
    foreach($comprovantes as $comp){
        ...
        if($comp > 0){
            ...
            if(strtolower($_POST['gerarComprovantes']) == 'pc'){
                ...
                $conteudo .= "<p>RIFA ONLINE<br>BILHETE DFISICO-IGITAL</p>";
                ...
                $data_compra = date('d/m/Y', strtotime($compra['comp_dttransacao']));
                $conteudo .= "<p>RESERVA DE BILHETES<br>------------------------<br>ID: {$compra['comp_cod']} - {$data_compra}<br><b>NÃO É VÁLIDO COMO COMPROVANTE</b></p><p>Essa RESERVA será CANCELADA em 72h caso não seja enviado COMPROVANTE de PAGAMENTO</p>";

                //$conteudo .= "<p>Obrigado por participar de nossa ACAO ENTRE AMIGOS! Sendo o ganhador você recebera um SMS lhe avisando!</p>";
                $conteudo .= "<p><hr></p>";

            } elseif(strtolower($_POST['gerarComprovantes']) == 'jspdf'){ 
                $conteudo .= "<p>RIFA ONLINE<br> BILHETE FISICO-DIGITAL</p>";
                $conteudo .= "<p>TITULO DA RIFA: <b>{$rifa['rifa_titulo']}</b><p>";
                $conteudo .= "<p>Sorteio: <b>{$dataSorteio}</b><p>";
                $conteudo .= "<p>Vendedor: <b>{$vendedor['usu_nome']}</b></p>";
                $conteudo .= "<p>Valor do Bilhete: <b>R$ ".number_format($rifa['rifa_valorbilhete'],2,',','.')."</b></p>";

                $conteudo .= "<p>Data/Hora: <b>{$data}</b><p>";
                $conteudo .= "<p>Nome: <b>{$usuario['usu_nome']}</b><p>";
                $conteudo .= "<p>Tel: <b>{$telefoneFormatado}</b><p>";
                $conteudo .= "<p>Bairro/Região: <b>{$usuario['usu_regiao']}</b></p>";
                $conteudo .= "<p>BILHETE(S):</p>";
                $conteudo .= "<p>[ $bilhete_str ] </p>";
                ...
                $conteudo .= "</p><p>A responsabilidade de entrega do premio será do promotor desta AÇÃO ENTRE AMIGOS</p>";
                $conteudo .= "<p>PROMOTOR: <b>{$rifa['usu_nome']}</b><br>";
                $conteudo .= "TEL: <b>".formataTelefone($rifa['usu_celular'])."</b></p>";
                ...
                //$conteudo .= "<p>Obrigado por participar de nossa AÇÃO ENTRE AMIGOS! Sendo o ganhador você recebera um SMS lhe avisando!</p>";
                $conteudo .= "<p><hr></p>";
            } else { ?>
...
Obrigado por participar de nossa ACAO ENTRE AMIGOS! Sendo o ganhador você recebera um SMS lhe avisando! 
    ...
    if(strtolower($_POST['gerarComprovantes']) == 'pc' || strtolower($_POST['gerarComprovantes']) == 'jspdf')
        die($conteudo);
    else
        die();
                    ...
                    ?>
                        <tr onclick="$(this).children().children().prop('checked', !$(this).children().children().prop('checked'));">
                            ...
                            <td class="text-right removerRelatorio form-inline">
                                ...
                                <a class="btn btn-default" href="javascript: enviarWhatsapp('+55<?php echo $resbil['tel_cliente']; ?>', '<?php echo $bilhetes; ?>', '<?php echo $resbil['cliente']; ?>', '<?php echo $resbil['comp_cod']; ?>');">Lembrete</a>
...
<script>
    ...
    window.onload = function(){
        function buscarrifas(rifa){
            ...
            $('#btn_pross').click(function(){
                ...
                .done(function(r){
                    console.log(r);
                    location.href='index.php?p=cadastro_cliente&origem=revenda';
                })
                .fail(function(r){
                    console.log(r);
                });
    ...
    function repetirVendas(prosseguir = false){
        ...
        if(prosseguir){
            ...
            if(destino.val() > 0){
                ...
                .done(function(r){
                    console.log(r);
                    window.open("index.php?p=carrinho&origem=revenda", '_blank');
                })
                .fail(function(r){
                    console.log(r);
                    window.open("index.php?p=carrinho&origem=revenda", '_blank');
    ...
    function enviarWhatsapp(telefone, bilhetes, nome, id) {

        let texto = `Olá, ${nome}. \nSua aposta: ${id} BILHETE(S): [${bilhetes}] está como RESERVADA.\nEfetue o PAGAMENTO e GARANTA sua Sorte!`;
