    ...
    if (isset($_POST['enviar']) && isset($_SESSION['usuario'])) {
        ...
        if (count($erro) == 0) {
            ...
            if ($sucesso) {
                ...
                $sqlInsertSituacaoRifa = "INSERT INTO tbl_situacao_rifa (
                        id_rifa, 
                        situacao_rifa
                        ) VALUES (
                            '{$rifa_codigo}',
                ...
                $situacaoRifaSalva = $mysqli->query($sqlInsertSituacaoRifa) or die($mysqli->error);
                if ($situacaoRifaSalva) {

                    if (strtotime(date('Y-m-d H:i:s')) < strtotime('2024-06-20 08:00')) {
                        if ($_SERVER['REMOTE_ADDR'] == '179.48.23.77') {
                            echo "<script>location.href='index.php?p=qr_pagamento_rifa&rifa_id=$rifa_codigo';</script>";
                        } else {
                            echo "<script>location.href='index.php?p=qr_pagamento_rifa&rifa_id=$rifa_codigo';</script>";
                        }
                    } else {
...
?>
    ...
    <?php if ($_SERVER['REMOTE_ADDR'] == "179.48.23.102") { ?>
        <div class="row">
            <div class="col-md-12">
                <div class="alert alert-warning">
                    <h4> Atenção! </h4>
                    <p>
                        Ao criar uma nova rifa é necessário efetuar o pagamento do QRCode gerado após cliar na opção Criar Rifa
                    </p>
                </div>
            </div>
        </div>
    ...
    <form action="" enctype="multipart/form-data" method="post">
        <div class="col-lg-6 text-left">
            ...
            <div class="form-group col-lg-6">
                ...
                <select onchange="javascript: formulario(this.value);" type="text" <?php if (isset($_GET['codigo'])) echo 'disabled';
                                                                                    else echo 'required'; ?> name="nbilhete" class=" form-control">
        ...
        <div id="dezena_bolao" class="<?php if ($_SESSION['nbilhete'] >= 100 && $_SESSION['dezenabolao']) echo '';
            ...
            <div class="form-group col-lg-12">
                <label for="coluna_grupo">
                    <input value="1" <?php if ($_SESSION['coluna_grupo'] == 1) echo 'checked'; ?> value="1" onchange="colunaGrupo(this);" name="coluna_grupo" id="coluna_grupo" type="checkbox"> Ativar Coluna Grupo <span id="coluna_grupo_qtd" class="<?php if ($_SESSION['coluna_grupo'] != 1) echo 'esconder'; ?> form-group form-inline">| Quantidade: <input type="number" value="<?php if (isset($_SESSION['coluna_grupo_qtd']) && intval($_SESSION['coluna_grupo_qtd']) > 0) echo $_SESSION['coluna_grupo_qtd'];
                                                                                                                                                                                                                                                                                                                                                                                        else echo 0; ?>" name="coluna_grupo_qtd" step="1" class="form-control"></span>
                </label>
            </div>
        ...
        <div id="banca_online" class="<?php if ($_SESSION['banca_online'] != 1 && ($_SESSION['nbilhete'] != 100 || $_SESSION['dezenabolao'])) echo 'esconder'; ?> col-lg-6 text-left">
            ...
            <div class="form-group">
                <label for="bo"><input value="1" <?php if ($_SESSION['banca_online'] == 1) echo 'checked';
                                                    else echo ''; ?> id="bo" onchange="mostrar_detalhes(this);" name="banca_online" type="checkbox"> Banca Online</label>
            </div>
