...
function str_in_array_de_keys($bil, $pad){
		$final = array();
			if(!$bil) return array();
			if(substr_count($bil, '-') == 0 && substr_count($bil, ',') == 0)
				return array(str_pad($bil, $pad, '0', STR_PAD_LEFT)=>true);
			...
			if(substr_count($bil, '-') == 1){
				...
				for($start; $start <= $end; $start++){
						$final[str_pad($start, $pad, '0', STR_PAD_LEFT)] = true;
				}
			}else{
				$var = explode(',', $bil);
				foreach($var as $v){
					if(is_numeric(trim($v))){
							$final[str_pad(intval($v), $pad, '0', STR_PAD_LEFT)] = true;
					}
				}
			}
...
if(isset($_POST['processar'])){
	...
	if(!empty($_POST['grupo_inicial']))
		$grupo_inicial = strtoupper($_POST['grupo_inicial']);
	else
		$grupo_inicial = "";

	if($_POST['bolaodezena'])
		$bolaodezena = $_POST['surpresinha'];
	else
		$bolaodezena = '';
	...
	/*$tmp = array_de_str_in_array_de_num($_POST['bilhete']);


	$tmp2 = verificar_bilhetes_repetidos($tmp);
	if(((string)$tmp2) != 'ok'){
		$resultado['sucesso'] = 0;
		$resultado['msg'] = "Erro! O bilhete $tmp2 está sendo repetido.";
		// limpar bilhetes repetidos
	...
	$arrayFinal = array();
	$size = strlen($numBilhetes)-1;

	foreach($_POST['revendedor'] as $k=>$rev){

		$tmp = str_in_array_de_keys($_POST['bilhete'][$k], $size);
		foreach($tmp as $bil=>$x){
			if(!isset($arrayFinal[$bil]))
				$arrayFinal[$bil] = true;
			else{
				$repetidos[] = $bil;
				unset($tmp[$bil]);
		...
		/*echo ((count(str_in_array_de_num($_POST['bilhete'][$k])) % $numeros_por_bilhete) > 0);
		echo "{$_POST['bilhete'][$k]} $rev";*/
...
<div class="col-lg-12">
	...
	<?php if ($dados_da_rifa['rifa_dono'] != $_SESSION['usuario'] || $_SESSION['admin'] == 'S') { ?>
...
<div class="col-lg-12 col-md-12">
	...
	<form id="salvar_form" action="">
		...
		<div class="form-group col-sm-4">
			<p><label><input name="ativar_sms" <?php if($res['ativar_sms']) echo 'checked="checked"'; ?> value="1" type="checkbox"> Ativar SMS</label></p>
			...
			<div class="form-inline form-group">
				<select name="surpresinha" id="surpresinha" <?php if(!$res['bolaodezena']) echo 'disabled'; ?>  class="form-control">
					...
					<option <?php if(intval($res['bolaodezena']) == 1000) echo 'selected'; ?> value="1000">1.000</option>
					<option <?php if(intval($res['bolaodezena']) == 10000) echo 'selected'; ?> value="10000">10.000</option>
				</select>
			</div>
		</div>
		<div class="col-sm-4">
			<div class="form-group">
				<label for="">Grupo Inicial</label>
				<input type="text" value="<?= $res['starting_group']; ?>" name="grupo_inicial" id="grupo_inicial" class="form-control">
			</div>
			<div class="form-group">
		...
		<?php } else { ?>
			<input name="embaralhamento" value="<?php if($res['embaralhar']) echo '1'; else echo ''; ?>" type="hidden">
			<input name="agrupamento" value="<?php if($res['agrupar']) echo '1'; else echo ''; ?>" type="hidden">
			<input value="<?php if($res['bolaodezena'] >= 100) echo '1'; else echo ''; ?>" name="bolaodezena" type="hidden">
			<?php if($res['bolaodezena']) { ?>
			<input name="surpresinha" type="hidden" id="surpresinha" value="<?= $res['bolaodezena']; ?>">
			<?php } ?>
			<input type="hidden" value="<?= $res['starting_group']; ?>" name="grupo_inicial" id="grupo_inicial">
			<input name="numeros_por_bilhete" value="<?= $res['numeros_por_bilhete']; ?>" id="numeros_por_bilhete" type="hidden">
...
<script>
function dezenaCheck(v){

	if(v){
		$('#surpresinha').attr('disabled', false);
	}else{
		
		$('#surpresinha').attr('disabled', true);
	}
}
...
function salvar(){
	console.log( $( '#salvar_form' ).serialize() );
	$('#resultado').html('Salvando... Aguarde!');
	$.post('page/revenda_loteamento.php', $( '#salvar_form' ).serialize())
	.done(function(r){
		console.log(r);
		r = JSON.parse(r);
		console.log(r);
