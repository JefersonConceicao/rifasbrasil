...
function finalizar_rifa($rifa_codigo, $mysqli){
	...
	if(count($lista_vencedores) > 0){
		...
		$msg_vencedor = "RifasBRASIL: Parabens, vc foi um dos ganhadores da rifa rifasbrasil.com.br/rifa/".$rifa_codigo.". Premiado(s): ".implode(", ", $lista_bilhetes).". Clique no link ou ligue ".limparNumero($rifa[usu_celular]);
	...
	if($rifa['rifa_maxbilhetes'] <= 100){
		if(count($lista_perdedores) > 0){
			...
			//SMS para Perdedores
			//echo "PERDEDORES";
			$msg_perdedor = "RifasBRASIL: Rifa rifasbrasil.com.br/rifa/".$rifa_codigo." finalizou, infelizmente vc nao ganhou. Premiado(s): ".implode(", ", $lista_bilhetes).". A proxima podera ser sua!";
