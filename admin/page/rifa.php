...
if ($_SESSION[admin] == 'S') {
    ...
    $rifa_code = "
        ...
        a.aguardando_envio 
        from tbl_rifas r 
        left join tbl_autorizar_sms a on a.rifa = r.rifa_cod  
        left join tbl_situacao_rifa tsr ON tsr.id_rifa = r.rifa_cod 
        WHERE r.rifa_finalizar IS NOT NULL
        AND (tsr.situacao_rifa = 2 OR tsr.id IS NULL)
        ";
    $rifa_query = $mysqli->query($rifa_code) or die($mysqli->error);
...
$rifa_code = "
    ...
    FROM 
        tbl_rifas r 
    LEFT JOIN 
        lote l ON l.rifa = r.rifa_cod 
    LEFT JOIN 
        (SELECT 
            id_rifa, MAX(situacao_rifa) AS situacao_rifa
        FROM 
            tbl_situacao_rifa 
        GROUP BY 
            id_rifa) tsr ON tsr.id_rifa = r.rifa_cod
    WHERE 
        r.rifa_deletada IS NULL 
        AND r.rifa_dono = '$_SESSION[usuario]'
        AND (tsr.situacao_rifa = 2 OR tsr.id_rifa IS NULL)
    ORDER BY 
        r.rifa_cod DESC;
";
...
<div class="col-lg-12 text-left">
    ...
    <?php if ($_SESSION['usuario'] == "199034") { ?>
        <div class="form-group">
            <button onclick="javascript: location.href='index.php?p=liberar_rifa_especifica';" class="btn btn-success">
                Liberar Rifa Especifica
            </button>
        </div>
    <?php } ?>
    ...
    <?php if ($_SESSION['usuario'] == "199034") { ?>
        <div class="form-group">
            <button onclick="javascript: location.href='index.php?p=configuracoes';" class="btn btn-warning">
                Configurações
            </button>
        </div>
    <?php } ?>
