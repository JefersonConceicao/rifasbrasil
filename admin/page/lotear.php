    ...
    select 
        ...
        (
            ...
            FROM 
                tbl_situacao_rifa 
            GROUP BY id_rifa
        ) tsr ON tsr.id_rifa = r.rifa_cod
        where rifa_dono = '$_SESSION[usuario]' 
        AND (tsr.situacao_rifa = 2 OR tsr.id_rifa IS NULL)
        and rifa_deletada is null 
        ORDER BY rifa_cod DESC
";

// Seleciona as Rifas
$rifas_usuario = db_select($mysqli, $sqlRifasUsuario);
