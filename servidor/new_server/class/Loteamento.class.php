...
class Loteamento{
		...
		*/
		...
		foreach($bilhetes as $str){
			...
			if(strlen($str['bilhete']) == 0){
				...
				$tmp = $this->shuffleIt($tmp, $rifa);

				foreach($tmp as $inicio){
		...
		/*if($rifa > 4037 || $rifa == 4005)
			return $this->suffleIt($array_final, $rifa);*/
	...
	function getBilhetes($rifa, $semhtml = false){
		...
		/*if($this->embaralhamento[$rifa])
			$bilhetesReservados = $this->shuffleIt($bilhetesReservados, $rifa);*/
		...
		foreach($bil as $val){
			...
			if($this->agrupamento[$rifa]){
				...
				if($this->layout == 6){
					...
					$bilhetes_finais[] = "<span class=\"milhar\">Nº ".str_pad($val, strlen($bilhetes)-1, '0', STR_PAD_LEFT)."</span><br><small>ID. $rifa | ".substr(md5($rifa.$val), 0, $tamCodigo).$extra."</small>";
			}
