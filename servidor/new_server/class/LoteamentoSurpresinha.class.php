...
class Loteamento{
		...
		"<!DOCTYPE html>
			<head>
				<meta charset=\"utf-8\">";
					...
					echo str_replace('bloco-branco', 'not-valid', $this->getCss());

				if($this->surpresinha > 0){
					echo '<style>.bloco-branco-dir {
					    line-height: 1em;
					    margin-left: 0;
					    top: 0;
					    left: 0;
	...
	function getBilhetes($rifa, $semhtml = false){
		...
		if($this->surpresinha > 0){
			...
			foreach($this->agrupamento as $rifa=>$void){
				$this->agrupamento[$rifa] = false;
			}
		}
		...
		// GROUP FIX
		//$bilhetesReservados = $this->getArrayDeBilhetesReservados($rifa, true, $semhtml);
		...
		foreach($bil as $val){
			...
			if($this->agrupamento[$rifa]){
				...
				if($this->layout == 6){
					...
					$bilhetes_finais[] = $titulo_grupos[$k]."-".str_pad($val, strlen($bilhetes)-1, '0', STR_PAD_LEFT)."-".substr(md5($rifa.$val), 0, $tamCodigo).$extra;
				else
					$bilhetes_finais[] = "<span class=\"grupo\">".$titulo_grupos[$k]."</span>-<span class=\"milhar\">Nº ".str_pad($val, strlen($bilhetes)-1, '0', STR_PAD_LEFT)."</span><br><small>ID. $rifa | ".substr(md5($rifa.$val), 0, $tamCodigo).$extra."</small>";
			}else{
				...
				/*if($this->layout == 6){
					...
					$bilhetes_finais[] = "-".str_pad($val, strlen($bilhetes)-1, '0', STR_PAD_LEFT)."-".substr(md5($rifa.$val), 0, $tamCodigo).$extra;
				else*/
				// gera as 10 dezenas
				$listaDezenas = array();
				for($random = 0; $random < 10; $random++){
					$listaDezenas[] = '['.str_pad(rand (0, 99), 2, '0', STR_PAD_LEFT).']';
				}
				$bilhetes_finais[] = "<span class=\"milhar\">".implode(' ', $listaDezenas)."</span><br><small>$val</small>";

				/* OLD $bilhetes_finais[] = "<span class=\"milhar\">Nº ".str_pad($val, strlen($bilhetes)-1, '0', STR_PAD_LEFT)."</span><br><small>ID. $rifa | ".substr(md5($rifa.$val), 0, $tamCodigo).$extra."</small>";*/
			}
